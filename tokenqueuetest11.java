...
@DisplayName("TokenQueue CSS Functionality")
public class TokenQueueCssTest {

    @Nested
    @DisplayName("escapeCssIdentifier()")
    class EscapeCssIdentifierTest {
        @ParameterizedTest(name = "[{index}] input=`{1}`, expected=`{0}`")
        @MethodSource("org.jsoup.parser.TokenQueueCssTest#escapeCssIdentifierTestCases")
        void shouldCorrectlyEscapeCssIdentifier(String expected, String input) {
    ...
    class ConsumeCssIdentifierTest {
        @ParameterizedTest(name = "[{index}] input=`{1}`, expected=`{0}`")
        @MethodSource("org.jsoup.parser.TokenQueueCssTest#consumeCssIdentifierTestCases")
        void shouldCorrectlyConsumeCssIdentifier(String expected, String cssIdentifier) {
    ...
    @Nested
    @DisplayName("consumeElementSelector()")
    class ConsumeElementSelectorTest {
        @Test
        @DisplayName("should stop at a dangling escape character at EOF")
        void stopsAtDanglingEscape() {
            TokenQueue q = new TokenQueue("Foo\\");
            String tagName = q.consumeElementSelector();
            assertEquals("Foo", tagName);
        }
    }

    // Data providers are static and can be top-level for use by nested classes.

    // Sourced from: https://github.com/web-platform-tests/wpt/blob/328fa1c67bf5dfa6f24571d4c41dd10224b6d247/css/cssom/escape.html
    static Stream<Arguments> escapeCssIdentifierTestCases() {
        return Stream.concat(
            Stream.of(
                ...
                // A digit after a letter is not escaped
                ...
                // A digit after a hyphen at the start is escaped
                Arguments.of("-\\30 a",                           "-0a"),
                Arguments.of("-\\31 a",                           "-1a"),
                Arguments.of("-\\32 a",                           "-2a"),
                Arguments.of("-\\33 a",                           "-3a"),
            ...
            // Additional jsoup-specific tests
            Stream.of(
    ...
    // Sourced from:
    // 1. https://github.com/web-platform-tests/wpt/blob/36036fb5212a3fc15fc5750cecb1923ba4071668/dom/nodes/ParentNode-querySelector-escapes.html
    // 2. Chromium source: https://goo.gl/3Cxdov
    static Stream<Arguments> consumeCssIdentifierTestCases() {
        return Stream.concat(
            Stream.of(
                // Hex digit escapes
                ...
                // Hex digit special replacements (invalid sequences)
