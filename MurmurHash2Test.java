...
class MurmurHash2Test {
    ...
    /** Murmur 32-bit hash results using default seed (0x9747b28c). */
    ...
    /** Murmur 32-bit hash results using custom seed (0x71b4954d). */
    ...
    /** Murmur 64-bit hash results using default seed (0xe17a1465). */
    ...
    /** Murmur 64-bit hash results using custom seed (0x344d1f5c). */
    ...
    /** Test text for non-parameterized tests. */
    static final String text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit";

    /** Provides indices for parameterized tests. */
    static IntStream indices() {
        return IntStream.range(0, input.length);
    }

    @ParameterizedTest
    @MethodSource("indices")
    void hash32_WithDefaultSeed(int index) {
        final byte[] data = input[index];
        final int expected = results32_standard[index];
        final int hash = MurmurHash2.hash32(data, data.length);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash32 result for input %d: 0x%08x instead of 0x%08x", index, hash, expected));
    }

    @ParameterizedTest
    @MethodSource("indices")
    void hash32_WithCustomSeed(int index) {
        final byte[] data = input[index];
        final int seed = 0x71b4954d;
        final int expected = results32_seed[index];
        final int hash = MurmurHash2.hash32(data, data.length, seed);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash32 result for input %d with seed 0x%08x: 0x%08x instead of 0x%08x",
                index, seed, hash, expected));
    }

    @Test
    void hash32_StringWithDefaultSeed() {
        final int expected = 0xb3bf597e;
        final int hash = MurmurHash2.hash32(text);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash32 for string: 0x%08x instead of 0x%08x", hash, expected));
    }

    @Test
    void hash32_StringSubstringWithDefaultSeed() {
        final int expected = 0x4d666d90;
        final int hash = MurmurHash2.hash32(text, 2, text.length() - 4);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash32 for substring: 0x%08x instead of 0x%08x", hash, expected));
    }

    @ParameterizedTest
    @MethodSource("indices")
    void hash64_WithDefaultSeed(int index) {
        final byte[] data = input[index];
        final long expected = results64_standard[index];
        final long hash = MurmurHash2.hash64(data, data.length);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash64 result for input %d: 0x%016x instead of 0x%016x", index, hash, expected));
    }

    @ParameterizedTest
    @MethodSource("indices")
    void hash64_WithCustomSeed(int index) {
        final byte[] data = input[index];
        final int seed = 0x344d1f5c;
        final long expected = results64_seed[index];
        final long hash = MurmurHash2.hash64(data, data.length, seed);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash64 result for input %d with seed 0x%08x: 0x%016x instead of 0x%016x",
                index, seed, hash, expected));
    }

    @Test
    void hash64_StringWithDefaultSeed() {
        final long expected = 0x0920e0c1b7eeb261L;
        final long hash = MurmurHash2.hash64(text);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash64 for string: 0x%016x instead of 0x%016x", hash, expected));
    }

    @Test
    void hash64_StringSubstringWithDefaultSeed() {
        final long expected = 0xa8b33145194985a2L;
        final long hash = MurmurHash2.hash64(text, 2, text.length() - 4);
        assertEquals(expected, hash,
            () -> String.format("Unexpected hash64 for substring: 0x%016x instead of 0x%016x", hash, expected));
    }
}