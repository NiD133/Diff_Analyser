...
/**
 * Tests for CSS identifier escaping and parsing in {@link TokenQueue}.
 * <p>
 * Many test cases are sourced from the Web Platform Tests (WPT) to ensure compliance with CSS standards.
 * </p>
 * @see <a href="https://github.com/web-platform-tests/wpt/blob/master/css/cssom/escape.html">WPT CSSOM Escape Tests</a>
 * @see <a href="https://github.com/web-platform-tests/wpt/blob/master/dom/nodes/ParentNode-querySelector-escapes.html">WPT Query Selector Escape Tests</a>
 */
@DisplayName("TokenQueue CSS Identifier Handling")
public class TokenQueueCssIdentifierTest {

    // region escapeCssIdentifier Tests
    @DisplayName("Should correctly escape CSS identifiers")
    @ParameterizedTest(name = "[{index}] input=`{1}`, expected=`{0}`")
    @MethodSource("escapeCssIdentifierParameters")
    ...
    private static Stream<Arguments> escapeCssIdentifierParameters() {
        return Stream.of(
            // WPT cases from https://github.com/web-platform-tests/wpt/blob/328fa1c67bf5dfa6f24571d4c41dd10224b6d247/css/cssom/escape.html
            escapeNullBytes(),
            escapeReplacementCharacters(),
            escapeNumericPrefixes(),
            preserveLetterNumericSequences(),
            escapeDashNumericPrefixes(),
            preserveDoubleDashPrefixes(),
            escapeControlAndSpecialCharacters(),
            preserveValidCharacters(),
            escapeBackslashes(),
            preserveHighCodepoints(),
            escapeSingleDashAndOtherSymbols(),
            preserveAstralSymbolsAndLoneSurrogates(),
            // Additional jsoup-specific cases
            escapeSpecialIdentifierCharacters()
        ).flatMap(s -> s);
    }

    private static Stream<Arguments> escapeNullBytes() {
        return Stream.of(
            Arguments.of("", ""),
            Arguments.of("\uFFFD", "\0"),
            Arguments.of("a\uFFFD", "a\0"),
    ...
    private static Stream<Arguments> escapeReplacementCharacters() {
        return Stream.of(
            Arguments.of("\uFFFD", "\uFFFD"),
            Arguments.of("a\uFFFD", "a\uFFFD"),
            Arguments.of("\uFFFDb", "\uFFFDb"),
    ...
    private static Stream<Arguments> escapeNumericPrefixes() {
        return Stream.of(
    ...
    private static Stream<Arguments> preserveLetterNumericSequences() {
        return Stream.of(
    ...
    private static Stream<Arguments> escapeDashNumericPrefixes() {
        return Stream.of(
            Arguments.of("-\\30 a", "-0a"), Arguments.of("-\\31 a", "-1a"), Arguments.of("-\\32 a", "-2a"),
            Arguments.of("-\\33 a", "-3a"), Arguments.of("-\\34 a", "-4a"), Arguments.of("-\\35 a", "-5a"),
            Arguments.of("-\\36 a", "-6a"), Arguments.of("-\\37 a", "-7a"), Arguments.of("-\\38 a", "-8a"),
    ...
    private static Stream<Arguments> preserveDoubleDashPrefixes() {
        return Stream.of(Arguments.of("--a", "--a"));
    }

    private static Stream<Arguments> escapeControlAndSpecialCharacters() {
        return Stream.of(
    ...
    private static Stream<Arguments> preserveValidCharacters() {
        return Stream.of(
    ...
    private static Stream<Arguments> escapeBackslashes() {
        return Stream.of(Arguments.of("hello\\\\world", "hello\\world"));
    }

    private static Stream<Arguments> preserveHighCodepoints() {
        return Stream.of(Arguments.of("hello\u1234world", "hello\u1234world"));
    }

    private static Stream<Arguments> escapeSingleDashAndOtherSymbols() {
        return Stream.of(
            Arguments.of("\\-", "-"),
            Arguments.of("\\ \\!xy", "\u0020\u0021\u0078\u0079")
        );
    ...
    private static Stream<Arguments> preserveAstralSymbolsAndLoneSurrogates() {
        return Stream.of(
            Arguments.of("\uD834\uDF06", "\uD834\uDF06"), // astral symbol (U+1D306)
            Arguments.of("\uDF06", "\uDF06"), // lone surrogates
            Arguments.of("\uD834", "\uD834")
        );
    }

    private static Stream<Arguments> escapeSpecialIdentifierCharacters() {
        return Stream.of(
    ...
    // region consumeCssIdentifier Tests
    @DisplayName("Should correctly parse (consume) CSS identifiers")
    @ParameterizedTest(name = "[{index}] input=`{1}`, expected=`{0}`")
    @MethodSource("consumeCssIdentifierParameters")
    void testConsumeCssIdentifier(String expected, String cssIdentifier) {
        assertParsedCssIdentifierEquals(expected, cssIdentifier);
    }

    private static Stream<Arguments> consumeCssIdentifierParameters() {
        return Stream.of(
            // WPT cases from https://github.com/web-platform-tests/wpt/blob/36036fb5212a3fc15fc5750cecb1923ba4071668/dom/nodes/ParentNode-querySelector-escapes.html
            consumeHexDigitEscapes(),
            consumeSpecialHexDigitReplacements(),
            consumeGeneralEscapes(),
            consumeNonEdgeCases(),
            // Chromium cases from https://goo.gl/3Cxdov
            consumeChromiumIdentTests(),
            // Additional jsoup-specific cases
            consumeAdditionalIdentifierCases()
        ).flatMap(s -> s);
    }

    private static Stream<Arguments> consumeHexDigitEscapes() {
        return Stream.of(
    ...
    private static Stream<Arguments> consumeSpecialHexDigitReplacements() {
        return Stream.of(
            // 1. zero points
    ...
    private static Stream<Arguments> consumeGeneralEscapes() {
        return Stream.of(
    ...
    private static Stream<Arguments> consumeNonEdgeCases() {
        return Stream.of(
    ...
    private static Stream<Arguments> consumeChromiumIdentTests() {
        return Stream.of(
    ...
    private static Stream<Arguments> consumeAdditionalIdentifierCases() {
        return Stream.of(
            Arguments.of("1st", "\\31\r\nst"),
            Arguments.of("1", "\\31\r"),
    ...
    @Test
    @DisplayName("unescape should correctly handle consecutive backslashes")
    public void unescapeHandlesConsecutiveBackslashes() {
