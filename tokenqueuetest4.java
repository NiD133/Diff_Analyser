...
/**
 * Tests for CSS-related methods in TokenQueue, such as identifier escaping and consumption.
 * Test cases are sourced from the Web Platform Tests (WPT) and Chromium test suites to ensure spec compliance.
 */
public class TokenQueueCssTest {

    // These data providers are static and reside in the top-level class.
    // Nested test classes access them via a fully qualified name.

    static Stream<Arguments> escapeCases() {
        return Stream.concat(webPlatformEscapeCases(), additionalEscapeCases());
    }

    /**
     * Test cases for escaping CSS identifiers, based on the W3C Web Platform Tests.
     ...
     */
    private static Stream<Arguments> webPlatformEscapeCases() {
        return Stream.of(
            // Null bytes are replaced with the replacement character U+FFFD.
            Arguments.of("", ""),
            Arguments.of("\uFFFD", "\0"),
            Arguments.of("a\uFFFD", "a\0"),
            Arguments.of("\uFFFDb", "\0b"),
            ...
            // Existing replacement characters are preserved.
            Arguments.of("\uFFFD", "\uFFFD"),
            Arguments.of("a\uFFFD", "a\uFFFD"),
            Arguments.of("\uFFFDb", "\uFFFDb"),
            ...
            // Digits that are not at the start of an identifier are not escaped.
            ...
            // A hyphen followed by a digit at the start of an identifier is escaped.
            Arguments.of("-\\30 a", "-0a"), Arguments.of("-\\31 a", "-1a"), Arguments.of("-\\32 a", "-2a"),
            Arguments.of("-\\33 a", "-3a"), Arguments.of("-\\34 a", "-4a"), Arguments.of("-\\35 a", "-5a"),
            Arguments.of("-\\36 a", "-6a"), Arguments.of("-\\37 a", "-7a"), Arguments.of("-\\38 a", "-8a"),
            Arguments.of("-\\39 a", "-9a"),

            // A double-dash prefix is not escaped.
            Arguments.of("--a", "--a"),

            // Various other characters and sequences.
            ...
            // Astral symbols (like U+1D306) and lone surrogates are preserved.
            Arguments.of("\uD834\uDF06", "\uD834\uDF06"),
            Arguments.of("\uDF06", "\uDF06"),
            Arguments.of("\uD834", "\uD834")
        );
    ...
    private static Stream<Arguments> additionalEscapeCases() {
        return Stream.of(
            // Characters that have special meaning in CSS selectors are escaped.
            Arguments.of("one\\#two\\.three\\/four\\\\five", "one#two.three/four\\five"),

            // Valid identifiers that don't need escaping.
            Arguments.of("-a", "-a"),
            Arguments.of("--", "--")
        );
    }

    static Stream<Arguments> consumeCases() {
        return Stream.concat(webPlatformConsumeCases(), additionalConsumeCases());
    }

    /**
     * Test cases for consuming CSS identifiers, based on WPT and Chromium tests.
     ...
     * @see <a href="https://cs.chromium.org/chromium/src/third_party/blink/renderer/core/css/css_grammar_test.cc">Chromium CSS Grammar Tests</a>
     */
    private static Stream<Arguments> webPlatformConsumeCases() {
        return Stream.of(
            // Escaped hex digits are unescaped.
            ...
            // Zero, surrogate, and out-of-range code points are replaced with U+FFFD.
            ...
            // Other escaped characters are unescaped.
            ...
            Arguments.of("continueEscapes00", "continueEscapes\\30 \\30"),
            Arguments.of("continueEscapes00", "continueEscapes\\30\\30 "),
            Arguments.of("continueEscaes00", "continueEscapes\\30\\30"), // Typo in original test, preserved for history
    ...
    private static Stream<Arguments> additionalConsumeCases() {
        return Stream.of(
            // Hex escape followed by newline.
            Arguments.of("1st", "\\31\r\nst"),
            // Hex escape followed by carriage return (which is consumed).
            Arguments.of("1", "\\31\r"),
            // Hex escape followed by carriage return and a character.
    ...
    class EscapeCssIdentifier {
        @ParameterizedTest(name = "[{index}] input=\"{1}\" -> \"{0}\"")
        @MethodSource("org.jsoup.parser.TokenQueueCssTest#escapeCases")
        void escapesAccordingToSpec(String expected, String input) {
    ...
    class ConsumeCssIdentifier {
        @ParameterizedTest(name = "[{index}] parsing \"{1}\" -> \"{0}\"")
        @MethodSource("org.jsoup.parser.TokenQueueCssTest#consumeCases")
        void consumesAccordingToSpec(String expected, String cssToParse) {
    ...
    @Nested
    @DisplayName("TokenQueue.unescape()")
    class Unescape {
        @Test
        @DisplayName("should unescape backslashed characters like parentheses and other backslashes")
        void unescapesSpecialChars() {
