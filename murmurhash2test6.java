...
/**
 * Tests MurmurHash2 with a set of known inputs and expected hash values.
 * These test vectors verify the correctness of both 32-bit and 64-bit hash implementations
 * with both default and custom seeds.
 */
@DisplayName("MurmurHash2 Known-Answer Tests")
class MurmurHash2Test {

    /**
     * A special seed value used for testing non-default seed hash calculations.
     */
    private static final int SPECIAL_TEST_SEED = 0x344d1f5c;

    // Test data sources below are structured as parallel arrays.
    // The same index across all arrays corresponds to a single test case.

    /**
     * Random input data with various lengths, from 16 bytes down to 0.
     */
    ...
    private static final int[] EXPECTED_32BIT_DEFAULT_SEED = {0x96814fb3, 0x485dcaba, 0x331dc4ae, 0xc6a7bf2f, 0xcdf35de0, 0xd9dec7cc, 0x63a7318a, 0xd0d3c2de, 0x90923aef, 0xaf35c1e2, 0x735377b2, 0x366c98f3, 0x9c48ee29, 0x0b615790, 0xb4308ac1, 0xec98125a, 0x106e08d9};

    /** Expected 32-bit hash results for the INPUTS using the SPECIAL_TEST_SEED. */
    private static final int[] EXPECTED_32BIT_CUSTOM_SEED = {0xd92e493e, 0x8b50903b, 0xc3372a7b, 0x48f07e9e, 0x8a5e4a6e, 0x57916df4, 0xa346171f, 0x1e319c86, 0x9e1a03cd, 0x9f973e6c, 0x2d8c77f5, 0xabed8751, 0x296708b6, 0x24f8078b, 0x111b1553, 0xa7da1996, 0xfe776c70};

    /** Expected 64-bit hash results for the INPUTS using the default library seed. */
    private static final long[] EXPECTED_64BIT_DEFAULT_SEED = {0x4987cb15118a83d9L, 0x28e2a79e3f0394d9L, 0x8f4600d786fc5c05L, 0xa09b27fea4b54af3L, 0x25f34447525bfd1eL, 0x32fad4c21379c7bfL, 0x4b30b99a9d931921L, 0x4e5dab004f936cdbL, 0x06825c27bc96cf40L, 0xff4bf2f8a4823905L, 0x7f7e950c064e6367L, 0x821ade90caaa5889L, 0x6d28c915d791686aL, 0x9c32649372163ba2L, 0xd66ae956c14d5212L, 0x38ed30ee5161200fL, 0x9bfae0a4e613fc3cL};

    /** Expected 64-bit hash results for the INPUTS using the SPECIAL_TEST_SEED. */
    private static final long[] EXPECTED_64BIT_CUSTOM_SEED = {0x0822b1481a92e97bL, 0xf8a9223fef0822ddL, 0x4b49e56affae3a89L, 0xc970296e32e1d1c1L, 0xe2f9f88789f1b08fL, 0x2b0459d9b4c10c61L, 0x377e97ea9197ee89L, 0xd2ccad460751e0e7L, 0xff162ca8d6da8c47L, 0xf12e051405769857L, 0xdabba41293d5b035L, 0xacf326b0bb690d0eL, 0x0617f431bc1a8e04L, 0x15b81f28d576e1b2L, 0x28c1fe59e4f8e5baL, 0x694dd315c9354ca9L, 0xa97052a8f088ae6cL};

    // --- Test Case Providers ---

    static Stream<Arguments> defaultSeed32BitCases() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], EXPECTED_32BIT_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> customSeed32BitCases() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], EXPECTED_32BIT_CUSTOM_SEED[i]));
    }

    static Stream<Arguments> defaultSeed64BitCases() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], EXPECTED_64BIT_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> customSeed64BitCases() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], EXPECTED_64BIT_CUSTOM_SEED[i]));
    }

    // --- Test Methods ---

    @ParameterizedTest(name = "hash32(input[{index}], defaultSeed) -> {1}")
    @MethodSource("defaultSeed32BitCases")
    void testHash32WithDefaultSeed(final byte[] data, final int expectedHash) {
        ...
        assertEquals(expectedHash, actualHash,
            () -> String.format("Hash mismatch for input of length %d. Expected 0x%08x, got 0x%08x",
                data.length, expectedHash, actualHash));
    }

    @ParameterizedTest(name = "hash32(input[{index}], customSeed) -> {1}")
    @MethodSource("customSeed32BitCases")
    void testHash32WithCustomSeed(final byte[] data, final int expectedHash) {
        final int actualHash = MurmurHash2.hash32(data, data.length, SPECIAL_TEST_SEED);
        assertEquals(expectedHash, actualHash,
            () -> String.format("Hash mismatch for input of length %d. Expected 0x%08x, got 0x%08x",
                data.length, expectedHash, actualHash));
    }

    @ParameterizedTest(name = "hash64(input[{index}], defaultSeed) -> {1}")
    @MethodSource("defaultSeed64BitCases")
    void testHash64WithDefaultSeed(final byte[] data, final long expectedHash) {
        ...
        assertEquals(expectedHash, actualHash,
            () -> String.format("Hash mismatch for input of length %d. Expected 0x%016x, got 0x%016x",
                data.length, expectedHash, actualHash));
    }

    @ParameterizedTest(name = "hash64(input[{index}], customSeed) -> {1}")
    @MethodSource("customSeed64BitCases")
    void testHash64WithCustomSeed(final byte[] data, final long expectedHash) {
        final long actualHash = MurmurHash2.hash64(data, data.length, SPECIAL_TEST_SEED);
        assertEquals(expectedHash, actualHash,
            () -> String.format("Hash mismatch for input of length %d. Expected 0x%016x, got 0x%016x",
                data.length, expectedHash, actualHash));
    }
