...
/**
 * Tests MurmurHash2 against a known set of input vectors and expected hash values.
 */
@DisplayName("MurmurHash2")
class MurmurHash2Test {

    /**
     * A known seed value used to generate the "seed" test vectors.
     * The corresponding expected hash arrays were generated using this seed.
     */
    private static final int CUSTOM_TEST_SEED = 0x12345678;

    // --- Test Data: A corpus of byte arrays with varying lengths ---
    ...
    // --- Expected Hashes for TEST_INPUTS ---
    ...
    static Stream<Arguments> provide32BitDefaultSeedTestCases() {
        return IntStream.range(0, TEST_INPUTS.length)
            .mapToObj(i -> Arguments.of(TEST_INPUTS[i], EXPECTED_HASHES_32_BIT_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> provide32BitCustomSeedTestCases() {
        return IntStream.range(0, TEST_INPUTS.length)
            .mapToObj(i -> Arguments.of(TEST_INPUTS[i], EXPECTED_HASHES_32_BIT_CUSTOM_SEED[i]));
    }

    static Stream<Arguments> provide64BitDefaultSeedTestCases() {
        return IntStream.range(0, TEST_INPUTS.length)
            .mapToObj(i -> Arguments.of(TEST_INPUTS[i], EXPECTED_HASHES_64_BIT_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> provide64BitCustomSeedTestCases() {
        return IntStream.range(0, TEST_INPUTS.length)
            .mapToObj(i -> Arguments.of(TEST_INPUTS[i], EXPECTED_HASHES_64_BIT_CUSTOM_SEED[i]));
    }

    @DisplayName("should produce known 32-bit hash for a given string")
    @Test
    void hash32OfStringShouldReturnKnownValue() {
        final String text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit";
        final int expectedHash = 0xb3bf597e;

        final int actualHash = MurmurHash2.hash32(text);

        assertEquals(expectedHash, actualHash);
    }

    @DisplayName("should produce known 32-bit hashes for byte arrays with default seed")
    @ParameterizedTest(name = "Input length: {0}.length")
    @MethodSource("provide32BitDefaultSeedTestCases")
    void hash32OfByteArrayWithDefaultSeedShouldReturnKnownValues(final byte[] data, final int expectedHash) {
        final int actualHash = MurmurHash2.hash32(data, data.length);
        assertEquals(expectedHash, actualHash);
    }

    @DisplayName("should produce known 32-bit hashes for byte arrays with a custom seed")
    @ParameterizedTest(name = "Input length: {0}.length")
    @MethodSource("provide32BitCustomSeedTestCases")
    void hash32OfByteArrayWithCustomSeedShouldReturnKnownValues(final byte[] data, final int expectedHash) {
        final int actualHash = MurmurHash2.hash32(data, data.length, CUSTOM_TEST_SEED);
        assertEquals(expectedHash, actualHash);
    }

    @DisplayName("should produce known 64-bit hashes for byte arrays with default seed")
    @ParameterizedTest(name = "Input length: {0}.length")
    @MethodSource("provide64BitDefaultSeedTestCases")
    void hash64OfByteArrayWithDefaultSeedShouldReturnKnownValues(final byte[] data, final long expectedHash) {
        final long actualHash = MurmurHash2.hash64(data, data.length);
        assertEquals(expectedHash, actualHash);
    }

    @DisplayName("should produce known 64-bit hashes for byte arrays with a custom seed")
    @ParameterizedTest(name = "Input length: {0}.length")
    @MethodSource("provide64BitCustomSeedTestCases")
    void hash64OfByteArrayWithCustomSeedShouldReturnKnownValues(final byte[] data, final long expectedHash) {
