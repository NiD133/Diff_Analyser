...
/**
 * Tests for TokenQueue's CSS identifier parsing and escaping.
 * Includes tests derived from W3C Web-Platform-Tests and other edge cases.
 */
...
public class TokenQueueCssIdentifierTest {
    ...
    class EscapeCssIdentifier {
        ...
        @DisplayName("should correctly escape various CSS identifiers")
        @ParameterizedTest(name = "input=`{1}`, expected=`{0}`")
        @MethodSource("parameters")
        void shouldCorrectlyEscape(String expected, String input) {
         ...
         */
        private static Stream<Arguments> webPlatformTests() {
            return Stream.of(
                ...
                // Identifiers with a letter then a number are not escaped
                ...
                // Double-dash prefixes are valid and not escaped
                Arguments.of("--a", "--a"),
        ...
        private static Stream<Arguments> additionalEdgeCases() {
            return Stream.of(
    ...
    class ConsumeCssIdentifier {
        ...
        @DisplayName("should correctly parse various CSS identifiers")
        @ParameterizedTest(name = "input=`{1}`, expected=`{0}`")
        @MethodSource("parameters")
        void shouldCorrectlyConsume(String expected, String cssIdentifier) {
            TokenQueue queue = new TokenQueue(cssIdentifier);
            String consumed = queue.consumeCssIdentifier();
            assertEquals(expected, consumed);
        }

        static Stream<Arguments> parameters() {
            return Stream.concat(
                webPlatformAndChromiumTests(),
         ...
         * - Chromium CSS tests: https://goo.gl/3Cxdov
         */
        private static Stream<Arguments> webPlatformAndChromiumTests() {
            return Stream.of(
                // Hex digit escapes
                ...
                // Out of range code point escapes
                ...
                // Non-ASCII and high code points
                Arguments.of("\u2003", "\u2003"),
                Arguments.of("\u00A0", "\u00A0"),
                Arguments.of("\u1234", "\u1234"),
                Arguments.of("\uD808\uDF45", "\uD808\uDF45"),
        ...
        private static Stream<Arguments> additionalParsingTests() {
            return Stream.of(
                Arguments.of("1st", "\\31\r\nst"),
                Arguments.of("1", "\\31\r"),
    ...
    @Test
    @DisplayName("chompBalanced() should correctly handle escaped delimiters")
    void chompBalancedWithEscapedDelimiters() {
        // This test ensures that chompBalanced correctly handles escaped delimiters.
        // The backslashes before parentheses `\(` and `\)` should be treated as
        // literal characters, not as balancing delimiters.
        String input = ":contains(one (two) \\( \\) \\) three) four";
        TokenQueue queue = new TokenQueue(input);

        // Consume the part before the opening parenthesis
        String pre = queue.consumeTo("(");
        // Chomp the balanced section
        ...
        assertEquals("one (two) \\( \\) \\) three", balanced, "The balanced string should retain its internal escapes.");
        assertEquals("one (two) ( ) ) three", TokenQueue.unescape(balanced), "The unescaped string should be correct.");
