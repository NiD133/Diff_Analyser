 ...
 * This ensures the implementation is stable and correct across different data lengths.
 */
@DisplayName("MurmurHash2")
class MurmurHash2Test {

    /**
     * A seed used for testing non-default seed hash calculations.
     */
    private static final int TEST_SEED = 0x12345678;

    // Test data fixtures: Input byte arrays and their expected hash results.
    ...
    /**
     * Expected 32-bit MurmurHash results using the default library seed.
     */
    private static final int[] RESULTS_32_DEFAULT_SEED = {
    ...
    /**
     * Expected 32-bit MurmurHash results using a custom test seed.
     */
    private static final int[] RESULTS_32_CUSTOM_SEED = {
    ...
    /**
     * Expected 64-bit MurmurHash results using the default library seed.
     */
    private static final long[] RESULTS_64_DEFAULT_SEED = {
    ...
    /**
     * Expected 64-bit MurmurHash results using a custom test seed.
     */
    private static final long[] RESULTS_64_CUSTOM_SEED = {
    ...
    static Stream<Arguments> hash32DefaultSeedParameters() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], RESULTS_32_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> hash32CustomSeedParameters() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], RESULTS_32_CUSTOM_SEED[i]));
    }

    static Stream<Arguments> hash64DefaultSeedParameters() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], RESULTS_64_DEFAULT_SEED[i]));
    }

    static Stream<Arguments> hash64CustomSeedParameters() {
        return IntStream.range(0, INPUTS.length)
            .mapToObj(i -> Arguments.of(INPUTS[i], RESULTS_64_CUSTOM_SEED[i]));
    }

    // --- Test Cases ---

    @ParameterizedTest
    @MethodSource("hash64DefaultSeedParameters")
    @DisplayName("hash64(byte[], length) should produce known values")
    void hash64WithDefaultSeed_producesKnownValues(final byte[] data, final long expectedHash) {
        final long actualHash = MurmurHash2.hash64(data, data.length);
        assertEquals(expectedHash, actualHash);
    }

    @ParameterizedTest
    @MethodSource("hash64CustomSeedParameters")
    @DisplayName("hash64(byte[], length, seed) should produce known values")
    void hash64WithCustomSeed_producesKnownValues(final byte[] data, final long expectedHash) {
        final long actualHash = MurmurHash2.hash64(data, data.length, TEST_SEED);
        assertEquals(expectedHash, actualHash);
    }

    @ParameterizedTest
    @MethodSource("hash32DefaultSeedParameters")
    @DisplayName("hash32(byte[], length) should produce known values")
    void hash32WithDefaultSeed_producesKnownValues(final byte[] data, final int expectedHash) {
        final int actualHash = MurmurHash2.hash32(data, data.length);
        assertEquals(expectedHash, actualHash);
    }

    @ParameterizedTest
    @MethodSource("hash32CustomSeedParameters")
    @DisplayName("hash32(byte[], length, seed) should produce known values")
    void hash32WithCustomSeed_producesKnownValues(final byte[] data, final int expectedHash) {
